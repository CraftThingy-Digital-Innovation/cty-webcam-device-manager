# @craftthingy-digital-innovation/cty-webcam-device-manager-web
...
### Instalasi
```bash
npm install @craftthingy-digital-innovation/cty-webcam-device-manager-web
```

### Cara Penggunaan
```javascript
import { WebcamDeviceManager } from '@craftthingy-digital-innovation/cty-webcam-device-manager-web';

const manager = new WebcamDeviceManager({
  videoElement: document.getElementById('webcam-video'),
  canvasElement: document.getElementById('webcam-canvas'),
  onDevicesFound: (devices) => {
    // Dipanggil saat list kamera berhasil diambil
    devices.forEach(device => {
      console.log(`Kamera: ${device.label} (ID: ${device.deviceId})`);
    });
  },
  onError: (err) => {
...
// 1. Dapatkan list kamera yang ada
await manager.getDevices();

// 2. Mulai streaming kamera pertama
await manager.start();

// 3. Ambil snapshot frame gambar
const dataUrl = manager.capture('image/jpeg', 0.95);
console.log("Base64 Image:", dataUrl);

// 4. Hentikan kamera saat ditutup
manager.stop();
```
...
await manager.getDevices();
await manager.start(devices[0].deviceId);
